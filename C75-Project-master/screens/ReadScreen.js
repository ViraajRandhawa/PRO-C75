...
export default class ReadStoryScreen extends React.Component{
  ...
  componentDidMount = async () => {
    const allStories = await db.collection("books").limit(10).get();
  ...
  fetchMoreStories = async () => {
    const story = await db.collection("books").where('storyName', '==', text).startAfter(this.state.lastVisibleStory).limit(10).get();
    story.docs.map((doc) => {
      this.setState({
        book: [...this.state.book, doc.data()],
        lastVisibleStory: doc
